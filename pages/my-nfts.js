...
export default function MyAssests() {
  ...
  async function loadNFTs(){
    ...
    const data = await marketContract.fecthMyNFTs()
  ...
  if(loadingState == 'loaded' && !nfts.length) return (<h1
    className='px-20 py-7 text-4x1 text-red-600'>You do not own any NFTs currently :(</h1>)
                ...
                nfts.map((nft, i) => (
                  <div key={1} className='border shadow rounded-x1 overflow-hidden'>
                      ...
                      <div className='p-4 bg-purple-700'>
                        <p style={{height: '64px'}} className='text-4xl font-bold'>{
                            nft.name}</p>
